...
public class Huffman_coding
{
	...
	A min1,min2;
	int[] d=new int[100];
	Huffman_coding(int i,A min1,A min2)
	{
		heap_size=i;
		this.min1=min1;
		this.min2=min2;
	...
	void huffman_decoding(A root,int top) // O(n)
	{
	...
	void huffman_decoding2(A root, String code) // O(n)
	{
		if(root.left == null && root.right == null) {
			System.out.println(root.ch + " : " + code);
			return;
		}
		huffman_decoding2(root.left, code + 0);
		huffman_decoding2(root.right, code + 1);
	}
	
	
	A huffman_coding(A[] ob) // O(nlogn)
	{
		while(heap_size>1)
		{
	...
	public static void main(String args[])
	{
		...
		Huffman_coding x=new Huffman_coding(n,null,null);
