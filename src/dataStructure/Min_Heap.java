...
public class Min_Heap {
	...
	private static int[] increaseKey(int[] arr, int indx, int higherVal) { // Time = O(log(n))
		if(indx < 1 || higherVal >= arr[indx]) {
			return arr;
		}
		arr[indx] = higherVal;
		arr = minHeapify(arr, indx);
		return arr;
	}

	private static int[] decreaseKey(int[] arr, int index, int lowerValue) { // Time = O(log(n))
		if(index < 1 || lowerValue >= arr[index]) {
			return arr;
		}
	...
	private static int[] deleteMin(int[] arr) { // Time = O(log(n))
		arr[1] = arr[heap_size];
		heap_size--;
		arr = minHeapify(arr, 1);
		return arr;
	...
	private static int[] minHeapify(int[] arr, int index) { // Time = O(log(n))
		...
		if(index_of_left_child <= heap_size && arr[index_of_left_child] < arr[index]) {
			index_of_smallest_value = index_of_left_child;
		} else {
			index_of_smallest_value = index;
		}
		
		if(index_of_right_child <= heap_size && arr[index_of_right_child] < arr[index_of_smallest_value]) {
			index_of_smallest_value = index_of_right_child;
		}
		
		if(index_of_smallest_value != index) {
			int temp = arr[index_of_smallest_value];
			arr[index_of_smallest_value] = arr[index];
			arr[index] = temp;
			arr = minHeapify(arr, index_of_smallest_value);
		}
		return arr;
	}
}
